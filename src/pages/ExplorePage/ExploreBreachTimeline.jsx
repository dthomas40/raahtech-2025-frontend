...
import "./ExploreBreachTimeline.css";
...
const ExploreBreachTimeline = () => {
  ...
  // animate detail on active breach change
  useEffect(() => {
  ...
  // responsive breach buttons per line
  useEffect(() => {
  ...
  // ESC closes modal + prevent body scroll when open
  ...
  const handleClick = (breach) => setActiveBreach(breach);

  const groupedBreaches = [];
  for (let i = 0; i < breaches.slice(breaches.length - breachCountPerLine, breaches.length).length; i += breachCountPerLine) {
    groupedBreaches.push(
      breaches
        .slice(breaches.length - breachCountPerLine, breaches.length)
        .slice(i, i + breachCountPerLine)
    );
  }

  return (
    <div className='ExploreBreachTimeline'>

      {groupedBreaches.slice(0, 1).map((group, groupIndex) => (
        <div key={groupIndex}>
          <div className={`ExploreBreachLine Line${groupIndex + 1}`}>
            {group.map((breach, index) => (
              <div
                className={`ExploreBreach ExploreBreach${groupIndex * breachCountPerLine + index + 1} ${activeBreach === breach ? 'active' : ''}`}
                key={index}
                onClick={() => handleClick(breach)}
              >
                <div className='ExploreBreachButton' title={`${breach.year}-${breach.company}`}>
                  <img src={BreachButtonGraphic} alt={`${breach.company} ${breach.year}`} />
                </div>
                <span className='ExploreBreachYearLabel'>{breach.year}</span>
              </div>
            ))}
            <div className='ExploreBreachLineConnector' />
          </div>
        </div>
      ))}

      <div className={`ExploreBreachContent ${activeBreach ? "visible" : ""} ${animateDetail ? 'ReusableFadeUp' : ''}`}>
        {activeBreach && (
          <div className="ExploreBreachDetails">
            <h2>{activeBreach.year}</h2>
            <h2>{activeBreach.company}</h2>
            <p>{activeBreach.description}</p>

            <div className='ExploreBreachDetailsCTAContainer'>
              <button
                className='ExploreBreachDetailsCTA'
                onClick={() => setIsModalOpen(true)}
                aria-haspopup="dialog"
                aria-expanded={isModalOpen}
              >
                See How RAAH Prevents This <img className="ExploreBreachDetailsCTAArrow" src={CTAArrow} alt="" />
              </button>
            </div>
          </div>
        )}
      ...
      {isModalOpen && activeBreach && (
        <div
          className='ExploreBreachModalOverlay'
          role='dialog'
          aria-modal='true'
          aria-label={`How RAAH prevents: ${activeBreach.company}`}
          onClick={() => setIsModalOpen(false)}
        >
          <div className='ExploreBreachModal' onClick={(e) => e.stopPropagation()}>
            <div className='ExploreBreachModalHeader'>
              <h3>How RAAH Prevents This — {activeBreach.company}</h3>
              <button
                className='ExploreBreachModalClose'
                onClick={() => setIsModalOpen(false)}
                aria-label='Close'
                title='Close'
              >
            ...
            <div className='ExploreBreachModalBody'>
              {activeBreach.raah?.length ? (
                <ul className='ExploreBreachModalList'>
                  {activeBreach.raah.map((item, i) => (
                    <li key={i} className='ExploreBreachModalItem'>
                      <h4 className='ExploreBreachModalItemTitle'>{item.categoryTitle}</h4>
                      <p className='ExploreBreachModalItemDesc'>{item.categoryDescription}</p>
                    </li>
                  ))}
                </ul>
              ) : (
...
export default ExploreBreachTimeline;