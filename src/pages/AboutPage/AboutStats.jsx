...
const AboutStats = () => {
    ...
    return (
        <div className='AboutStatsContainer' ref={containerRef}>
            <div className='AboutStatsWrapper'>
                {stats.map((stat, index) => (
                    <div className='AboutStatsContent' key={index}>
                        <FontAwesomeIcon icon={stat.icon} className="AboutStatsIcon" />
                        <h3 className='AboutStatsTitle'>
                            {Math.floor(counts[index])}{stat.suffix}
                        </h3>
                        <p className='AboutStatsDescription'>{stat.description}</p>
                    </div>
                ))}
            </div>
        </div>
