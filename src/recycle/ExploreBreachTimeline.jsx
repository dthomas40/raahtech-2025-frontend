...
const ExploreBreachTimeline = () => {
    // const [activeBreach, setActiveBreach] = useState(breaches[breaches.length - 1]);
    ...
    // useEffect(() => {
    //     setActiveBreach(breaches[breaches.length - 1]);
    // }, [])
    ...
    for (let i = 0; i < breaches.slice(breaches.length - breachCountPerLine, breaches.length).length; i += breachCountPerLine) {
        groupedBreaches.push(breaches.slice(breaches.length - breachCountPerLine, breaches.length).slice(i, i + breachCountPerLine));
    }
    ...
    return (
        <div className='ExploreBreachTimeline'>
            ...
            {groupedBreaches.slice(0, 1).map((group, groupIndex) => (
                <div key={groupIndex}>
                    <div className={`ExploreBreachLine Line${groupIndex + 1}`} key={groupIndex}>
            ...
            <div className={`ExploreBreachContent ${activeBreach ? "visible" : ""} ${animateDetail ? 'ReusableFadeUp' : ''}`}>
                ...
                {activeBreach && <div className="ExploreBreachDetails">
                    ...
                    <div className='ExploreBreachDetailsCTAContainer'>
                        <button className='ExploreBreachDetailsCTA'>See How RAAH Prevents This <img className="ExploreBreachDetailsCTAArrow" src={CTAArrow} /></button>
                    </div>
                </div>}
            </div>
        </div>
